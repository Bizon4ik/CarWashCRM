...
public class ServiceServiceImpl implements ServiceService {
    ...
    @Autowired
    public ServiceServiceImpl(DaoFactory daoFactory) {
        this.daoFactory = daoFactory;
    }

    @Override
    public void addServiceName(ServiceName serviceName) throws SQLException {
        serviceName.setName(serviceName.getName().trim().toLowerCase());
        serviceName.setDateOfCreation(new Date());

        daoFactory.getServiceDao().addServiceName(serviceName);
