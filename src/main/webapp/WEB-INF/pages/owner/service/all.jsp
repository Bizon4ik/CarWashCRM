...
<div class="container-fluid" id="body">
  ...
  <div class="row">
    <div class="col-xs-12 col-md-10 col-md-offset-1">
      <p class="userTitle">Все услуги: </p>
    </div>
  </div>
  ...
  <div class="row">
    ...
    <form action="/owner/service/delete" method="post">
      </c:if>
      ...
      <table class="table-bordered table-hover table-striped col-xs-12 table" >
        ...
        <tbody>
        <c:forEach items="${serviceNameList}" var="row" varStatus="loop">
          <tr>
            <td>${loop.count}</td>
            <td>${row.name}</td>
            <td>${row.countable ? "+" : "-"}</td>
            ...
            <td>${row.id}</td>
            <c:if test="${delete != null}">
              <td><input type="checkbox" name="listIdService" value="${row.id}"/></td>
            </c:if>
          </tr>
